...
public class CacheClient {
   ...
   // private static final ExecutorService CACHE_REBUILD_EXECUTOR = Executors.newFixedThreadPool(10);
    ...
    // 逻辑过期
    public <R, ID> R queryWithLogicalExpireCount(
            String keyPrefix, ID id, Class<R> type, Function<ID, R> dbFallback, Long time, TimeUnit unit) {
        String key = keyPrefix + id;
        // 1.从redis查询商铺缓存
        String json = stringRedisTemplate.opsForValue().get(key);
        ...
        if (StrUtil.isBlank(json)) {
            ...
            Boolean expire =  stringRedisTemplate.opsForValue().setIfAbsent("repetition:"+id, id.toString(), 1, TimeUnit.MINUTES);
            if(expire){
                R newR = dbFallback.apply(id);
                // 重建缓存
                this.setWithLogicalExpire(key, newR, time, unit);
    ...
    public void deleteCache(String key,String id){
        String a= key+id;
        Boolean delete = stringRedisTemplate.delete(a);
        System.out.println(delete);
        stringRedisTemplate.delete("repetition:" + id);
    }
}