...
public class AdUserDetailServiceImpl implements UserDetailsService {
   ...
   private TCompanyLoginApi tCompanyLoginApi;
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        //将来连接数据库根据账号查询用户信息
        TCompany tCompany = tCompanyLoginApi.selectOne(username);
        if(tCompany == null){
        ...
        //根据用户的id查询用户的权限
        List<TPermission> permissions = tCompanyLoginApi.rolePermission(tCompany.getCompanyId());
        ...
        loginUser.setId(tCompany.getCompanyId());
        loginUser.setUsername(tCompany.getContactName());
        //loginUser
        loginUser.setUserMobile(tCompany.getContactPhone());
        String principal = JSON.toJSONString(loginUser);

        UserDetails userDetails = User.withUsername(principal).password(tCompany.getPassword()).authorities(permissionArray).build();
        return userDetails;
    }
