 ...
 */
@Service
public class AdRecommendDetailServiceImpl implements UserDetailsService {
   ...
   private TCompanyLoginApi tCompanyLoginApi;
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        //将来连接数据库根据账号查询用户信息
        TRecommend tRecommend = tCompanyLoginApi.selectOneRecommend(username);
        if(tRecommend == null){
            //如果用户查不到，返回null，由provider来抛出异常
            throw  new  UsernameNotFoundException("该用户不存在");
        }
        if(tRecommend.getStatus().equals("1")){
            throw new InvalidClientException("用户已经被禁用");
        }
        //根据用户的id查询用户的权限
        List<TPermission> permissions = tCompanyLoginApi.rolePermissionRecommend(tRecommend.getId());
        ...
        loginUser.setId(tRecommend.getId());
        loginUser.setUsername(tRecommend.getUsername());
        //loginUser
        loginUser.setUserMobile(tRecommend.getPhone());
        String principal = JSON.toJSONString(loginUser);

        UserDetails userDetails = User.withUsername(principal).password(tRecommend.getPassword()).authorities(permissionArray).build();
        return userDetails;
    }
