 ...
 */
@Service
public class JBWXLoginDetailServiceImpl implements UserDetailsService {
   ...
   private TCompanyLoginApi tCompanyLoginApi;
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        //将来连接数据库根据账号查询用户信息
        TJobhunter wxtJobhunter = tCompanyLoginApi.getWXTJobhunter(username);
        if(wxtJobhunter == null){
            //如果用户查不到，返回null，由provider来抛出异常
            throw  new  UsernameNotFoundException("该用户不存在");
        }
        if(wxtJobhunter.getStatus().equals("1")){
            throw new InvalidClientException("用户已经被禁用");
        }
        //根据用户的id查询用户的权限
        List<TPermission> permissions = tCompanyLoginApi.rolePermissionRecommend(wxtJobhunter.getUserId());
        ...
        loginUser.setId(wxtJobhunter.getUserId());
        loginUser.setUsername(wxtJobhunter.getUserName());
        //loginUser
        loginUser.setUserMobile(wxtJobhunter.getPhoneNumber());
        String principal = JSON.toJSONString(loginUser);

        UserDetails userDetails = User.withUsername(principal).password(wxtJobhunter.getPassword()).authorities(permissionArray).build();
        return userDetails;
    }
